 ...
 */
function transformKeys(o, transformCallback) {
    // length / void obj check, returns an error if object is empty.
    if (o === {}) {
        throw new Error(`Object should not be empty, obtained: ${JSON.stringify(o)}`);
    }
    let newObject = {};
    let keys = [];
    let values = [];
    ...
    try {
        ...
        throw new Error(`Error occurred while parsing your object's keys ${JSON.stringify(e)}`);
    }
    // Values splitting
    try {
        ...
        throw new Error(`Error occurred while parsing your object's values ${JSON.stringify(e)}`);
    }
    // Callback applier
    keys.forEach((key) => {
